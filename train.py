...
from datasets.dataset import TrainDataset,\
                                ...
                                BusiTestDataset,\
                                BrainMRITestDataset
from datasets.optimized_dataset import OptimizedTrainDataset
import pprint
...
def main(args):
    ...
    if "optimized" in args.config_path:
        train_dataset = OptimizedTrainDataset(args=args.config,
                                        source=os.path.join(args.config.data_root,args.config.train_dataset),
                                        preprocess=preprocess,
                                        k_shot=args.k_shot)
        # Using a more conservative learning rate for optimized training
        lr = 0.0005
    else:
    ...
    optimizer = torch.optim.AdamW([
            {'params': prompt_maker.prompt_learner.parameters(),'lr': lr},
            {'params': adapter.parameters(),"lr": lr},
            {'params': map_maker.parameters(),"lr": lr},  # Learnable layer fusion weights
        ], lr=lr, weight_decay=1e-4)

    scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=(args.config.epoch - 5) * len(train_dataloader))
    
    if args.config.epoch > 5:
        # 5-epoch linear warmup
        scheduler_warmup = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, end_factor=1.0, total_iters=5 * len(train_dataloader))
        scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[scheduler_warmup, scheduler_cosine], milestones=[5 * len(train_dataloader)])
    else:
    ...
    for epoch in range(0, args.config.epoch):
        ...
        if (epoch+1) % args.config.val_freq_epoch == 0:
            ...
            if save_flag:
                ...
                best_record['no_improve'] = 0
            else:
                # Early Stopping Logic
                if 'no_improve' not in best_record:
                    best_record['no_improve'] = 0
                best_record['no_improve'] += 1
                
                if best_record['no_improve'] >= 15:
    ...
    for test_dataset_name in test_dataloaders:
        ...
        with torch.no_grad():
            for i, input in enumerate(tqdm(test_dataloader,desc=test_dataset_name)):
                ...
                _, image_tokens_lr = clip_model.encode_image(images_lr, out_layers=args.config.layers_out)
                image_features_lr = necker(image_tokens_lr)
                vision_adapter_features_lr = adapter(image_features_lr)
                propmt_adapter_features_lr = prompt_maker(vision_adapter_features_lr)
                anomaly_map_lr = map_maker(vision_adapter_features_lr, propmt_adapter_features_lr)
                # Flip back
                anomaly_map_lr = torch.flip(anomaly_map_lr, dims=[3])

                # 3. Flip UD (New: Vertical Flip)
                images_ud = torch.flip(images, dims=[2]) # Dim 2 is Height
                _, image_tokens_ud = clip_model.encode_image(images_ud, out_layers=args.config.layers_out)
                image_features_ud = necker(image_tokens_ud)
                vision_adapter_features_ud = adapter(image_features_ud)
                propmt_adapter_features_ud = prompt_maker(vision_adapter_features_ud)
                anomaly_map_ud = map_maker(vision_adapter_features_ud, propmt_adapter_features_ud)
                # Flip back
                anomaly_map_ud = torch.flip(anomaly_map_ud, dims=[2])

                # 4. Average
                anomaly_map = (anomaly_map + anomaly_map_lr + anomaly_map_ud) / 3.0
                # ------------------------------------
                ...
                # Top-K average scoring (more robust than max)
                k = max(1, int(0.05 * H * W))  # Top 5% pixels
                topk_scores, _ = torch.topk(anomaly_map.view(B, -1), k, dim=-1)
                anomaly_score = topk_scores.mean(dim=-1)
