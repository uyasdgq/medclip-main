...
class Adapter(nn.Module):
    ...
    def __init__(self,
        ...
        for i,input_size in enumerate(input_sizes):
            # Enhanced adapter with bottleneck structure and GELU activation
            self.add_module("{}_adapter".format(i), nn.Sequential(
                nn.Conv2d(input_size, input_size // 4, 1, 1),
                nn.GELU(),
                nn.Conv2d(input_size // 4, target, 1, 1)
            ))
